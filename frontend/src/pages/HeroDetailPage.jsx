  ...
  return (
    <section className="space-y-4">
      ...
      <div className="card p-4 space-y-2">
        ...
        <div><b>Суперсили:</b> {Array.isArray(current.superpowers)? current.superpowers.map(p=><span key={p} className="badge mr-1">{p}</span>) : String(current.superpowers)}</div>
      </div>

      <div className="flex gap-2">
        <button className="btn btn-outline" onClick={onAddImageUrl}>Додати за URL</button>
        <label className="btn btn-outline cursor-pointer">Завантажити файл
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3">
        {current.images?.map(img=>(
          <div key={img.id} className="card p-2">
            ...
            <button className="btn btn-danger w-full mt-2" onClick={()=>dispatch(removeImage({id:current.id, imageId:img.id}))}>Прибрати</button>
          </div>
        ))}
      </div>
    </section>
